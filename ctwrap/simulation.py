...
see `adiabatic flame example <../examples/adiabatic_flame_example.ipynb>`_ or
`ignition_example <../examples/ignition_example.ipynb>`_ .

 *Note:* Other cantera simulations module can be used with this module provided they followed the
 format in `minimal <./modules/minimal.py>`_.
 """
...
class Simulation(object):
    ...
    Required module functions 'run', 'save' and 'defaults' are passed through
    ...
    def from_module(cls,
        ...
        .. code-block:: Python
           ...
           # creates a simulation object with module `minimal`
           sim = ctwrap.Simulation.from_module(cw.modules.minimal)

        Arguments:
           module (module): handle name or handle to module running the simulation
           output (dict):  Output information. Generated by `SimulationHandler`
        """
    ...
    def run(self, name: Optional[str] ='defaults',
        ...
        The :meth:`run` is used to call the module's run method.
        A simple usage is

        .. code-block:: Python

           # calls the run method in module 'minimal`
           sim.run()
    ...
    def _save(self, mode="a", task=None, **kwargs: str) -> None:
        ...
        The :meth:`_save` is used to call the module's save method.
    ...
    def _save_metadata(self,
        ...
        :py:func:`~ctwrap.parser.save_metadata`.
...
class SimulationHandler(object):
    ...
    .. note:: :class:`SimulationHandler` objects should be
        instantiated using the :meth:`from_yaml`.

    An example of a minimal yaml input is:

       .. code-block:: YAML

          output: # output file information
            format: null # no file output
    ...
    def from_yaml(cls, yaml_file: str,
        ...
        .. code-block:: Python
           ...
           sh = ctwrap.Simulation.from_module('minimal.yaml')
    ...
    def run_task(self, sim: Simulation, task: str, **kwargs:str):
        ...
        The :meth:`run_task` method calls the module's run and save method to run and
         ...
         :py:func:`~ctwrap.parser._save_metadata`. to save the metadata.
         A simple usage is:

         .. code-block:: Python

           # Runs the task `sleep_0.4` using `sim` object
           ctwrap.Simulation.run_task(sim, 'sleep_0.4" )
    ...
    def run_serial(self,
        ...
        file serially. A simple usage is:
    ...
    def run_parallel(self,
        ...
        file in parallel using python multiprocessing. A minimal example is:
